...
async def init_clients():
    ...
    if config.CHECK_ID_TO_BUY and config.BUY_GIFT:
        try:
            ...
            logger.critical(f'Incorrect ID_TO_BUY! Try subscribe to channel or send msg to buyer.{e}')
            exit()

    logger.info(f"Initialized account client. User: {user.full_name} (@{user.username}); id:{user.id}; Have premium: {'✅' if user.is_premium else '❌'}; star balance: {star_balance}; Id to buy gifts: {config.ID_TO_BUY}")
